...
import { isValidSuiObjectId, SUI_SYSTEM_STATE_OBJECT_ID } from '@onelabs/sui/utils';
...
export default function Sui({ coinId, objectId }: SuiProps) {
  ...
  const selectedUnstakingCoin = getSuiAccountAsset();
  ...
  const unstakeTx = useMemo<TransactionType | undefined>(() => {
    if (!currentUnstakeObjectId || !isValidSuiObjectId(currentUnstakeObjectId) || !currentStakerAddress) {
      return undefined;
    }
    ...
    tx.moveCall({
      ...
      arguments: [tx.object(SUI_SYSTEM_STATE_OBJECT_ID), tx.object(currentUnstakeObjectId)],
    });
  ...
  const errorMessage = useMemo(() => {
    ...
    if (gt(expectedBaseFeeAmount, baseAvailableAmount)) {
      return t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.insufficientFee');
    }
    ...
    if (dryRunTransaction?.result?.effects.status.status !== 'success') {
      return t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.failedToDryRun');
    }

    if (!debouncedTx) {
      return t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.failedToBuildTransaction');
    }
  ...
  return (
    <>
      <BaseBody>
        <>
          <CoinContainer>
            ...
            <CoinSymbolText variant="h2_B">{`${coinSymbol} ${t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.unstake')}`}</CoinSymbolText>
            <ChainNameContainer>
              <Typography variant="b3_M">
                {t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.stakingCoin', {
                  chainName: chainName,
                })}
              </Typography>
            </ChainNameContainer>
      ...
      <ReviewBottomSheet
        ...
        contentsTitle={
          selectedUnstakingCoin?.asset.symbol
            ? t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.unstakeReviewWithSymbol', {
                symbol: selectedUnstakingCoin.asset.symbol,
              })
            : t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.unstakeReview')
        }
        contentsSubTitle={t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.unstakeReviewDescription')}
        confirmButtonText={t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.unstake')}
      ...
      <TxProcessingOverlay
        ...
        title={t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.txProcessing')}
        message={t('pages.wallet.unstake.$coinId.$validatorAddress.Entry.Sui.index.txProcessingSub')}
      />
    </>
  );
}
